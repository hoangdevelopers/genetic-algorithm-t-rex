...
var game = (function(document) {
    ...
    function run() {
        ...
        console.log('eyePixel', eyePixel)
        if (isPixelEqual(eyePixel, C.blackPixel)) {
            currentDinoState = C.stateGround;
            console.log('stateGround')
        } else if (isPixelEqual(eyePixelDuck, C.blackPixel)) {
            currentDinoState = C.stateDuck;
            console.log('stateDuck')
        } else {
            currentDinoState = C.stateAirbone;
            console.log('stateAirbone')
        }

        var lookforwardDanger = false, distanceToInstance = 0;
        for (i = 0; i < currentLookAheadBuffer; i += 2) {
            if (isPixelEqual(getPixel(imageData, C.lookAheadX + i, C.lookAheadY), C.blackPixel)) {
                ...
                distanceToInstance = i;
                console.log('lookforwardDanger', i)
                break;
            }
        }
        ...
        console.log({
            ...
            distanceToInstance: distanceToInstance,
            // downClear: downClear,
